...
A small Electron playground that turns your MacBook lid into a controller. A Nothing-inspired home screen, live angle widgets, and small apps sharing one native input service.

![HyperHinge home screen](docs/screenshots/home.png)

## Small apps, one hinge
...
- **Accordion** — pretend you're a virtuoso. A real MIDI transcription of Julián Arcas’s _Soleá_ provides the melody, bass and harmony. Click Enable sound once; opening or closing the lid advances the score at your movement speed, and holding still silences it and holds your place. Use ←/→ to move between four-bar phrases, and ↑/↓ for octave shifts. No music theory required.

- **Laptop Pinball** — tilt a real 3D table with your lid. Close to roll away and open to brake or reverse. Hold two alternating targets to unlock the final hole before time runs out; overshooting either end loses the ball. Three courses tighten the precision and time requirements. Includes pause, reset, time-based star ratings and a comfortable neutral angle; no full closure needed. [Game rules and verification](docs/laptop-pinball.md).

- **The Other Side** — slowly close from 105° to 35° to lift an ordinary desktop and discover a procedural 3D miniature city, with warm windows, rooftops, trees, a fountain and moving cars. Reopen to conceal it. Input loss freezes the scene; reduced motion stops traffic.

**Build the next app.** A weird instrument, a physics toy, a tiny game, a useful accessibility experiment — contributions are welcome. The sensor, simulation, calibration and fullscreen are already handled. Start with the [contribution guide](CONTRIBUTING.md), [JS API](docs/native-api.md), and [example app](examples/angle-meter/AngleMeter.tsx).
...
With Make installed, `make` builds the production renderer and native helper
(`npm run build`), and `make dev` starts the Electron development app with Vite
(`npm run dev`). Run the install and asset setup commands above first.
...
| Enable accordion sound                | Click **Enable sound**                            |
